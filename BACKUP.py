...
def click_weekly():
    ...
    mycursor.execute(
        "Select TITLE, sum(Money) TOTAL_EXPENSE from " + en + " where DATE_OF_EXPENSE between curdate() - 7 and curdate() group by Title");
    ...
    x, p, texts = plt.pie(slices, colors=colors, radius=1.2, autopct="%1.1f%%",normalize=False)
...
def click_monthly():
    ...
    en = "expensetracker." + t.lower()
    mycursor.execute(
        "Select TITLE, sum(Money) TOTAL_EXPENSE from " + en + " where DATE_OF_EXPENSE between curdate() - 30 and curdate() group by Title");
...
def click_yearly():
    ...
    en = "expensetracker." + t.lower()
    mycursor.execute(
        "Select TITLE, sum(Money) TOTAL_EXPENSE from " + en + " where DATE_OF_EXPENSE between curdate() - 365 and curdate() group by Title");
    myresult = mycursor.fetchall()
